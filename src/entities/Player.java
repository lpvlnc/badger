...
public class Player extends Entity {
    ...
    public void weak() {
        if (poisonedParticle == null) {
            AudioPlayer.play(Audio.WEAK, Volume.NORMAL);
    ...
    public void useSteroid() {
        if (steroidCounter > 0 && !onSteroid && !weak) {
            AudioPlayer.play(Audio.USE_STEROID, Volume.NORMAL);
            steroidCounter--;
            startRunning();
            onSteroid = true;
        }
    ...
    public void takeDamage(int damage) {
        if (canBeDamaged) {
            AudioPlayer.play(Audio.PLAYER_HURT, Volume.NORMAL);
            if (weak) {
                life = 0;
                AudioPlayer.play(Audio.GAME_OVER, Volume.NORMAL);
                try {
